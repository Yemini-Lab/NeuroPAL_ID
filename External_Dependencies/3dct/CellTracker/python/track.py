...
def pr_gls_quick(X, Y, corr, BETA=300, max_iteration=20, LAMBDA=0.1, vol=1e8):
    ...
    Gram_matrix = np.exp(
        -np.sum(np.square(X_tile - X_tile.transpose(1, 0, 2)), axis=2)
        / (2 * BETA * BETA)
    )
    ...
    sigma_square = np.sum(np.sum(np.square(X_tile - Y_tile), axis=2)) / (
        3 * length_X * length_Y
    )
    ...
    for iteration in range(1, max_iteration):
        ...
        denominator = np.sum(P1, axis=1) + gamma * (2 * np.pi * sigma_square) ** 1.5 / (
            (1 - gamma) * vol
        )
...
def initial_matching_quick(ffn_model, ref, tgt, k_ptrs):
    ...
    for ref_i in range(ref.shape[0]):
        ...
        distance_ref, indices_ref = nbors_ref.kneighbors(
            ref[ref_i : ref_i + 1, :], return_distance=True
        )

        mean_dist_ref = np.mean(distance_ref)
        ref_x = (
            ref[indices_ref[0, 1 : k_ptrs + 1], :] - ref[indices_ref[0, 0], :]
        ) / mean_dist_ref
    ...
    for tgt_i in range(tgt.shape[0]):
        distance_tgt, indices_tgt = nbors_tgt.kneighbors(
            tgt[tgt_i : tgt_i + 1, :], return_distance=True
        )
        mean_dist_tgt = np.mean(distance_tgt)
        tgt_x = (
            tgt[indices_tgt[0, 1 : k_ptrs + 1], :] - tgt[indices_tgt[0, 0], :]
        ) / mean_dist_tgt
    ...
    tgt_x_flat_batch_meshgrid = (
        np.tile(tgt_x_flat_batch, (ref.shape[0], 1, 1))
        .transpose(1, 0, 2)
        .reshape((ref.shape[0] * tgt.shape[0], k_ptrs * 3 + 1))
    )

    corr = np.reshape(
        ffn_model.predict(
            [ref_x_flat_batch_meshgrid, tgt_x_flat_batch_meshgrid], batch_size=1024
        ),
...
def pr_gls(X, Y, corr, BETA=300, max_iteration=20, LAMBDA=0.1, vol=1e8):
    ...
    for idx_i in range(length_X):
        for idx_j in range(length_X):
            Gram_matrix[idx_i, idx_j] = np.exp(
                -np.sum(np.square(X[idx_i, :] - X[idx_j, :])) / (2 * BETA * BETA)
            )
    ...
    for iteration in range(1, max_iteration):
        ...
        for idx_Y in range(length_Y):
            ...
            denominator = denominator + gamma * (2 * np.pi * sigma_square) ** 1.5 / (
                (1 - gamma) * vol
            )
...
def initial_matching(fnn_model, ref, tgt, k_ptrs):
    ...
    for ref_i in range(ref.shape[0]):
        ...
        corr[:, ref_i] = np.reshape(
            fnn_model.predict([ref_x_flat_batch, tgt_x_flat_batch], batch_size=32),
            tgt.shape[0],
        )
...
def transform_cells(img3d, vectors3d):
    ...
    for label in range(1, img3d.max() + 1):
        ...
        image_temp = img3d[
            idx_1_start:idx_1_end, idx_2_start:idx_2_end, idx_3_start:idx_3_end
        ]
        idx_label = np.where(image_temp == label)
        output[
            idx_label[0] + idx_1_start + v1,
            idx_label[1] + idx_2_start + v2,
            idx_label[2] + idx_3_start + v3,
...
def plot_tracking_2d(T_ref, ax, draw_point, ref_ptrs, tgt_ptrs, x_axis, y_axis, sizes):
    ...
    for ptr_num in range(length_X):
        element.append(
            plot_arrow(
                ax,
                x1=ref_ptrs[ptr_num, x_axis] / sizes[0],
                y1=1 - ref_ptrs[ptr_num, y_axis] / sizes[1],
                x2=T_ref[ptr_num, x_axis] / sizes[0],
                y2=1 - T_ref[ptr_num, y_axis] / sizes[1],
            )
        )
    ax.axis("equal")
    return element
...
def plot_tracking_2d_realcoord(
    T_ref, ax, draw_point, ref_ptrs, tgt_ptrs, x_axis, y_axis
):
    ...
    for ptr_num in range(length_X):
        element.append(
            ax.arrow(
                x=ref_ptrs[ptr_num, x_axis],
                y=ref_ptrs[ptr_num, y_axis],
                dx=T_ref[ptr_num, x_axis] - ref_ptrs[ptr_num, x_axis],
                dy=T_ref[ptr_num, y_axis] - ref_ptrs[ptr_num, y_axis],
                color="C0",
...
def tracking_plot_xy(
    ax, ref_ptrs, tgt_ptrs, T_ref, yx_sizes, draw_point=True, layercoord=False
):
    """Draw the tracking process between two point sets in x-y plane"""
    x_axis = 1
    y_axis = 0
    if layercoord:
        element = plot_tracking_2d(
            T_ref, ax, draw_point, ref_ptrs, tgt_ptrs, x_axis, y_axis, yx_sizes
        )
    else:
        element = plot_tracking_2d_realcoord(
            T_ref, ax, draw_point, ref_ptrs, tgt_ptrs, x_axis, y_axis
        )
    return element


def tracking_plot_zx(
    ax, ref_ptrs, tgt_ptrs, T_ref, yz_sizes, draw_point=True, layercoord=True
):
    ...
    if layercoord:
        element = plot_tracking_2d(
            T_ref, ax, draw_point, ref_ptrs, tgt_ptrs, x_axis, y_axis, yz_sizes
        )
    else:
        element = plot_tracking_2d_realcoord(
            T_ref, ax, draw_point, ref_ptrs, tgt_ptrs, x_axis, y_axis
...
def FFN_matching_plot(ref_ptrs, tgt_ptrs, initial_match_score):
    ...
    for ptr_num in range(length_ref_ptrs):
        ...
        plt.plot(
            [ref_ptrs[cc_max_idx[1], 1], tgt_ptrs_y_bias[cc_max_idx[0], 1]],
            [-ref_ptrs[cc_max_idx[1], 0], -tgt_ptrs_y_bias[cc_max_idx[0], 0]],
            "r-",
        )
        cc_ref_tgt_temp[cc_max_idx[0], :] = 0
        cc_ref_tgt_temp[:, cc_max_idx[1]] = 0
...
def get_subregions(label_image, num):
    ...
    for label in range(1, num + 1):
        ...
        x_max, x_min, y_max, y_min, z_max, z_min = _get_coordinates(
            label, label_image, get_subregion=False
        )
        region_list.append(
            label_image[x_min : x_max + 1, y_min : y_max + 1, z_min : z_max + 1]
            == label
        )
...
def gaussian_filter(img, z_scaling=10, smooth_sigma=5):
    ...
    output_img = np.zeros(
        (shape_interp[0] + 10, shape_interp[1] + 10, shape_interp[2] + 10), dtype="int"
    )
    ...
    for label in range(1, np.max(img) + 1):
        ...
        (
            ...
            subregion_pad,
            voxels,
        ) = _get_coordinates(label, img_interp)
        ...
        output_img[x_min : x_max + 11, y_min : y_max + 11, z_min : z_max + 11] += (
            cell_region_interp * label
        )
        mask[x_min : x_max + 11, y_min : y_max + 11, z_min : z_max + 11] += (
            cell_region_interp * 1
        )
...
def _get_coordinates(label, label_image, get_subregion=True):
    ...
    if not get_subregion:
        ...
        subregion = np.zeros(
            (x_max - x_min + 11, y_max - y_min + 11, z_max - z_min + 11)
        )
        subregion[
            region[0] - x_min + 5, region[1] - y_min + 5, region[2] - z_min + 5
        ] = 0.5
