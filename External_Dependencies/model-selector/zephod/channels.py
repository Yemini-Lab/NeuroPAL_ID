...
def rl_deconvolution(img, sigma=5, gamma=1.0, lam=12, n_iter=10):
    ...
    for z in range(img.shape[0]):
        ...
        processed[z] = restoration.richardson_lucy(image_noisy, kernel[1], iterations=n_iter)
...
def threshold(img: np.ndarray, lo=50, hi=200) -> np.ndarray:
    processed = np.zeros_like(img)
    for z in range(img.shape[0]):
        ret, thresh = cv2.threshold(img[z], lo, hi, cv2.THRESH_BINARY)
        kernel = np.ones((1, 2), np.uint8)
        processed[z] = cv2.erode(thresh, kernel, iterations=1)
    return processed
...
def sharpen(img: np.ndarray,
    ...
    processed = np.zeros_like(img)
    for z in range(img.shape[0]):
        img_lut = apply_255_lut(img[z])
        processed[z] = cv2.filter2D(img_lut, -1, sharp)
    return processed
