classdef dlg
    ...
    methods (Static, Access = public)
        ...
        function handle = add_task(label)
            handle = Program.dlg.update_message(label, 'task');
        end

        function handle = step(label)
            handle = Program.dlg.update_message(label, 'step');
        end

        function handle = choice(message, choices)
            handle = Program.dlg.active();
            if ~isempty(handle)
                options = struct( ...
                    'Message', {message}, ...
                    'Options', {choices});
                Program.dlg.switch_to('choice', options);

            else
                handle = Program.dlg.create('choice', 'Message', message, 'Options', choices);
        ...
        function switch_to(mode, options)
            ...
            Program.dlg.cache(cache);
            delete(handle);
            Program.dlg.create(mode, options);
        end
        ...
        function resolve()
            ...
            if isa(handle, "matlab.ui.dialog.ProgressDialog")
                ...
                elseif Program.dlg.is_step(task_arr{end})
                    task_arr(end) = [];
                ...
                cache = Program.dlg.cache;

                if ~isempty(cache)
                   delete(handle)
                   Program.dlg.create('progress', cache);
                end
            end
        end
        ...
        function handle = create(mode, varargin)            
            ...
            switch mode
                case 'instruction'
                    handle = Program.dlg.create_instruction(p.Results);

                case 'progress'
                    handle = Program.dlg.create_progress(p.Results);

                case 'choice'
                    handle = Program.dlg.create_choice(p.Results);

                case 'warning'
                    handle = Program.dlg.create_warning(p.Results);

                case 'error'
                    handle = Program.dlg.create_error(p.Results);
            ...
            Program.dlg.active(handle);
        end
    end

    methods (Static, Access = private)
        function bool = is_task(str)
            identifiers = Program.dlg.identifiers;
