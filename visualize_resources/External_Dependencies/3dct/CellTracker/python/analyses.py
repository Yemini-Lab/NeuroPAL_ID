...
def get_activities(path_raw, path_tracked, volume_num, layer_num):
    ...
    for frame in range(1, volume_num + 1):
        ...
        if frame >= 2:
            images_label, images_raw = _read_image(
                frame, layer_num, path_raw, path_tracked
            )
        ...
        for label in range(1, cell_num + 1):
            ...
            activities[frame - 1, label - 1] = np.mean(
                intensity_label_i[sorted_intensity_idx[threshold:]]
            )
    return activities
...
def draw_signals(signals, ylim_upper=None, ylim_lower=None, figsize=(40, 20)):
    ...
    for row in range(row_n):
        for column in range(column_n):
            ...
            upper_sig_n, lower_sig_n = np.nanmax(signals[:, n]), np.nanmin(
                signals[:, n]
            )
    ...
    plt.subplots_adjust(
        left=0.02, bottom=0.02, right=0.98, top=0.98, wspace=0.2, hspace=0.2
    )
    return fig, axes
