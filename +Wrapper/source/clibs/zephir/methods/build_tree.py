...
def build_tree(
    ...
    if (sort_mode == 'depth' or sort_mode == 'similarity') and lr_coef > 0:
        ...
        d_full = get_all_pdists(dataset, shape_t, channel, pbar=True, filename=filename)
        if verbose:
    ...
    if d_full is not None:
        ...
        s_list = get_partial_pdists(dataset, shape_t, p_list, channel, pbar=True, filename=filename)
    else:
