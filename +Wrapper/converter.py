...
print(f"Loaded {len(video_neurons['worldline'])} worldlines.")

valid_worldlines = {}
invalid_worldlines = []

for i in tqdm(range(len(video_neurons['worldline'])), desc="Validating neuron structure...", leave=True):
    if isinstance(video_neurons['worldline'][i][0][0][0][0], str) and isinstance(video_neurons['worldline'][i][0][0][2], np.ndarray):
        ...
        valid_worldlines[video_neurons['worldline'][i][0][0][0][0]] = {
            ...
            'x': video_neurons['rois'][i][0][0][0],
            'y': video_neurons['rois'][i][0][0][1],
            'z': video_neurons['rois'][i][0][0][2]
        }
    else:
...
print(f"Ignoring worldlines {invalid_worldlines} due to lack of IDs.")
print(f"Validated {len(valid_worldlines.keys())}/{len(video_neurons['worldline'])} ({round((len(valid_worldlines.keys())/len(video_neurons['worldline']))*100)}%) worldlines.")
