...
def find_npal(cwd):
    local_dirs = [name for name in os.listdir(cwd) if os.path.isdir(name) or os.path.isdir(os.path.join(cwd, name))]
    if len(cwd) > 4:
        for eachDir in local_dirs:
            if 'NeuroPAL_ID' in eachDir:
                return os.path.join(cwd, eachDir, 'lib', 'ns')
        return find_npal(os.path.dirname(cwd))
    else:
        return False
...
def __get_resources():
    if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
        print('Loading HDMF namespaces in bundles state...')
        __location_of_this_file = Path(find_npal(os.getcwd()))
        print(f"Setting HDMF namespace_path to {__location_of_this_file}...")
    else:
        try:
