    ...
    for t in tpbar:
        ...
        with torch.no_grad():
            ...
            if lambda_d > 0 and zephod is not None:
                for _t in t_patch:
                    if filename is None:
                        data = n_io.get_data(dataset, _t, g=gamma, c=channel)
                    else:
                        data = n_io.get_data(dataset, _t, g=gamma, c=channel, filename=filename)
                    _pred = zephod(data)
            ...
            if filename is None:
                data = n_io.get_data(dataset, t_annot[root], g=gamma, c=channel)
            else:
                data = n_io.get_data(dataset, t_annot[root], g=gamma, c=channel, filename=filename)
